...
function SpawnMine(Pawn Target, vector TargetDir)
{
	...
	if (AvailableMines > 0)
	{
		...
		Mine.TossZ = 300.0;
		Mine.Init(TargetDir);
		//AvailableMines--; Unlimited mines for testing :D
		DeployedMines++;
	}
}
function CheckForEnemies()
{
	...
	{
		Level.Game.BroadCast(Controller, "ERROR - NOCONTROLLER", 'Say');
	...
	if (Team != Controller.PlayerReplicationInfo.Team.TeamIndex && Controller.PlayerReplicationInfo.Team != None)
	{
		...
		Level.Game.BroadCast(Controller, "ERROR - TEAM", 'Say');
		Destroy();
		return;
	}
	...
	if (AvailableMines + DeployedMines <= 0)
	{
		...
		Level.Game.BroadCast(Controller, "ERROR - ADDITION", 'Say');
		Destroy();
		return;
	}
	...
	if (!bDeleteMe)
	{
		...
		foreach RadiusActors(class'Pawn', P, DetectionRange, Location)
		{
			...
			else if (P.GetTeamNum() != Team && FastTrace(P.Location, Location))
			{
				SpawnMine(P, Normal(P.Location - Location));
				spawnedmine = true;
				break; // Only spawn one spider at a time.
...
DefaultProperties
{
	...
	// Sound.
	ActivateSnd(0) = Sound'UT3NightshadeBeta1.SpiderSounds.SpiderMine_Active01';
	ActivateSnd(1) = Sound'UT3NightshadeBeta1.SpiderSounds.SpiderMine_Active02';
	ActivateSnd(2) = Sound'UT3NightshadeBeta1.SpiderSounds.SpiderMine_Active03';
	DropSnd = Sound'UT3NightshadeBeta1.SpiderSounds.SpiderMine_Drop';
