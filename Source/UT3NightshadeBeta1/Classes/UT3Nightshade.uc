...
{
	// Only check for jump.
	if (Rise > 0 && Bot(Controller) == none)
	{
		if ((CurrentState == VS_Undeployed || CurrentState == VS_Cloaked) && IsOnGround() && CheckNearby() && NoObstacle() && bHasAmmo())
		{
			...
			if (ArmMine == none)
				Level.Game.BroadCast(Controller, "ArmMine == none", 'Say');
			...
			// @HACK: Set physics to PHYS_None to stop moving.
			SetPhysics(PHYS_None);
			
			KarmaParamsRBFull(KParams).KMaxSpeed = 0.0;
			
			// Prevents vehicle from turning up if deployed and trying to move backwards.
			PitchTorqueMax = 0.0;
			TurnTorqueMax = 0.0;
			RollTorqueMax = 0.0;
		}
...
function Fire(optional float F)
{
	...
	if (CurrentState == VS_Deployed)
	{
		...
		else if (Mines[2] == None && SelectedMine == 2)
		{
			...
			ShowMessage(0, 3); // "Only in next beta (have a shield :D)"
	...
	if (CurrentState == VS_Deployed)
	{
		switch (SelectedMine)
		{
			case 1:
				...
				SelectedMine = 2;
				PlaySound(Sound'UT3NightshadeBeta1.Sounds.SwitchDeployable', SLOT_None);
...
function bool ShouldDropDeployable()
{
	...
	b = Bot(Controller);
	if (b != None && Level.TimeSeconds - LastDropAttemptTime > 3.0)
	{
		if (b.Squad.IsDefending(Bot(Controller)) && CurrentState == VS_Deployed)
		{
			BotDropDeployable();
			return true;
		}
		else if (b.Enemy != None)
		{
			...
			if (CheckNearby() && VSize(EnemyDir) > 3000.0 && Normal(EnemyDir) dot Normal(B.Enemy.Velocity) > 0.5 && b.LineOfSightTo(b.Enemy))
			{
				BotDropDeployable();
				return true;
			}
...
DefaultProperties
{
	...
	// Sound.
	IdleSound = Sound'UT3NightshadeBeta1.Sounds.Engine';
	StartUpSound = Sound'UT3NightshadeBeta1.Sounds.EngineStart';
	ShutDownSound = Sound'UT3NightshadeBeta1.Sounds.EngineStop';
	DeploySnd = Sound'UT3NightshadeBeta1.Sounds.ArmExtend';
	UndeploySnd = Sound'UT3NightshadeBeta1.Sounds.ArmRetract';
	SwitchDeployableSnd = Sound'UT3NightshadeBeta1.Sounds.SwitchDeployable';
	DropItemSnd = Sound'UT3NightshadeBeta1.Sounds.DropItem';
